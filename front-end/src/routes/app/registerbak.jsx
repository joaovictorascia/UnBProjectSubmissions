...
export const Route = createFileRoute('/app/registerbak')({
...
function RegisterComponent() {
  ...
  const selectAccount = async (account) => {
    try {
      ...
      if (api) {
        ...
        // Método 1: Usar toHuman() e parsear (mais seguro)
        const balanceHuman = freeBalance.toHuman()
        const balanceNumber = parseFloat(balanceHuman.replace(/,/g, ''))

        // Método 2: Usar divisão segura com BigInt (para JavaScript moderno)
        let freeInTCESS
        try {
          // Converter para BigInt primeiro
          const balanceBigInt = freeBalance.toBigInt()
          const divisor = BigInt(Math.pow(10, tokenDecimals))
          freeInTCESS = Number(balanceBigInt / divisor) +
            Number(balanceBigInt % divisor) / Math.pow(10, tokenDecimals)
        } catch (bigIntError) {
          // Fallback: usar string parsing
          console.log('Using string parsing fallback')
        ...
        if (freeInTCESS < 100) {
          throw new Error(`Insufficient balance. You need at least 100 ${tokenSymbol} to register. Current: ${freeInTCESS.toFixed(2)} ${tokenSymbol}`)
        }
  ...
  // Versão melhorada da chargeTCESS
  const chargeTCESS = async (address, currentBalance) => {
    try {
      ...
      const balanceCheck = currentBalance
      const minRequired = 100

      if (balanceCheck < minRequired) {
        throw new Error(`Insufficient balance. Need ${minRequired} ${tokenSymbol}, you have ${balanceCheck.toFixed(2)}`)
      }

      setWalletStatus('Processing 100 TCESS registration fee...')

      // Para DEBUG: Mostrar quanto será debitado
      console.log(`Will charge: ${minRequired} ${tokenSymbol}`)
      console.log(`Current balance: ${balanceCheck} ${tokenSymbol}`)
      console.log(`Remaining after charge: ${(balanceCheck - minRequired).toFixed(2)} ${tokenSymbol}`)

      // Simulação com delay menor para melhor UX
      ...
      setWalletStatus(`✅ Connected & Charged: ${shortenedAddress} | -100 ${tokenSymbol}`)

      // Atualizar saldo local com precisão
      const remainingBalance = parseFloat((currentBalance - 100).toFixed(6))
      setUserBalance(remainingBalance)

      // Salvar no localStorage para referência
      localStorage.setItem('registrationFeePaid', 'true')
      localStorage.setItem('registrationFeeAmount', '100')
  ...
  // Função para transação REAL (quando configurar)
      ...
      return new Promise((resolve, reject) => {
        transfer.signAndSend(
          ...
          ({ status, events, txHash }) => {
            if (status.isInBlock) {
              setWalletStatus(`Transaction included in block...`)
            }
            ...
            if (status.isFinalized) {
              ...
              events.forEach(({ event }) => {
                if (api.events.system.ExtrinsicSuccess.is(event)) {
                  success = true
                  setTransactionHash(txHash.toHex())
                }
              })
    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`flex flex-col items-center ${tCessCharged ? 'text-green-400' : 'text-gray-400'}`}>
            ...
            <span className="text-xs">Pay 100 TCESS</span>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                {!showAccountList ? (
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-gray-400 text-center mb-2">Select your account:</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-green-400 text-sm">
                      ✓ 100 {tokenSymbol} fee paid • Balance: {userBalance?.toFixed(2)} {tokenSymbol}
                    </p>
                  </div>
                </div>
              </div>
        ...
        >
          {loading ? (
            <>
              <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBar="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
      ...
      <div className="mt-8 space-y-4">
        <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/30">
          ...
          <ul className="text-gray-400 text-sm space-y-1">
            <li className="flex items-center gap-2">
              ...
              <span>One-time fee: 100 {tokenSymbol}</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="text-purple-500">•</span>
              <span>Fee is charged immediately upon wallet connection</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="text-purple-500">•</span>
        ...
        <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/30">
          <h4 className="text-sm font-medium text-white mb-2">🔐 Wallet Security</h4>
          <ul className="text-gray-400 text-sm space-y-1">
            <li className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">•</span>
              <span>Your wallet address becomes your account ID</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">•</span>
            ...
            <li className="flex items-start gap-2">
              <span className="text-blue-500 mt-0.5">•</span>
              <span>Ensure you have Polkadot.js extension installed</span>
            </li>
          </ul>
          <div className="mt-3">
            <a
              ...
              className="inline-flex items-center gap-2 text-purple-400 hover:text-purple-300 text-sm hover:underline"
            >
        ...
        <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-700/30">
          ...
          <ul className="text-gray-400 text-sm space-y-1">
            ...
            <li className="flex items-start gap-2">
              <span className="text-orange-500 mt-0.5">•</span>
              <span>Username must be unique across the platform</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      {!api && (
        <div className="mt-6 p-4 bg-yellow-900/20 border border-yellow-700/30 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-medium text-white mb-1">Connecting to CESS Network</h4>
              <p className="text-gray-400 text-sm">
                Please wait while we establish connection to the blockchain...
              </p>
            </div>
          </div>
        </div>
